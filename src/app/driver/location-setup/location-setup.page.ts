    ...
    if ('geolocation' in navigator) {
      navigator.geolocation.getCurrentPosition(
        ...
        async (error) => {
          ...
          const alert = await this.alertController.create({
            header: 'GPS Error',
            message: 'Could not get your location. Please enable GPS or enter manually.',
            buttons: ['OK']
          });
          await alert.present();
        }
  ...
  async saveLocation() {
    if (this.locationForm.valid && (this.gpsEnabled || this.locationForm.value.manualAddress)) {
      ...
      setTimeout(async () => {
        ...
        const alert = await this.alertController.create({
          header: 'Success!',
          message: 'Location and preferences saved successfully!',
          buttons: [{
            text: 'Continue',
            handler: () => {
      ...
      const alert = await this.alertController.create({
        header: 'Incomplete',
        message: 'Please enable GPS or enter manual address, and fill all required fields.',
        buttons: ['OK']
      });
      await alert.present();
    }
